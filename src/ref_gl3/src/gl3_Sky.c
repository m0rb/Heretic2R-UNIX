#include "compat.h"
//
// gl3_Sky.c -- skybox clipping and rendering.
//
// H2 semantics from gl1_Sky.c (r_farclipdist / fog-dependent skybox scale,
// pics/skies/*.m8 sky faces, skyrotate-dependent seam clamp); draw technique
// from yq2 gl3_warp.c (MakeSkyVec into gl3_3D_vtx_t quads, model-matrix
// translate/rotate, si3Dsky program).
//
...
static void R_MakeSkyVec(float s, float t, const int axis, gl3_3D_vtx_t* vert) // YQ2: fills a vertex instead of glTexCoord2f() + glVertex3fv().
{
	// 1 = s, 2 = t, 3 = 2048
...
void R_DrawSkyBox(void)
{
	...
	hmm_mat4 model_mat = HMM_MultiplyMat4(old_model_mat, HMM_Translate(HMM_Vec3(r_origin[0], r_origin[1], r_origin[2])));

	if (skyrotate != 0.0f)
		model_mat = HMM_MultiplyMat4(model_mat, HMM_Rotate(r_newrefdef.time * skyrotate, HMM_Vec3(skyaxis[0], skyaxis[1], skyaxis[2])));

	gl3state.uni3DData.transModelMat4 = model_mat;
	GL3_UpdateUBO3D();
	...
	for (int i = 0; i < 6; i++)
	{
		...
		{
			GL3_BindTexnum(sky_images[skytexorder[i]]->texnum); // Q2: GL_Bind()
			...
			GL3_BufferAndDraw3D(sky_vertices, 4, GL_TRIANGLE_FAN); // gl1: GL_QUADS.
		}
	}
