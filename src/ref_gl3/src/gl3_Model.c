#include "compat.h"
//
// gl3_Model.c -- model loading and caching for the OpenGL 3.2 core renderer.
//
// Ported from gl1_Model.c (semantics preserved 1:1): Mod_ForName dispatch (BSP brush
// models, H2 sprites, H2 book models, FlexModel .fm files), BSP lump loaders, inline
// models, PVS decompression, registration sequence handling. Hunk usage is identical
// to gl1 (shared Hunk.c). RI_GetReferencedID() moved here from gl1_Main.c (gl3_Main.c
// is foundation-locked).
//
// Lightmap building (LM_*), warp surface subdivision (R_SubdivideSurface) and
// FlexModel block parsing (Mod_LoadFlexModel/Mod_RegisterFlexModel) are implemented
// by the world/flexmodel module ports - only the load dispatch lives here.
//
// Copyright 1998 Raven Software
//

#include "gl3_Model_internal.h"
...
// gl1: defined in gl1_Main.c (see gl3_Model_internal.h note).
...
// gl1: lives in gl1_Main.c; moved here because gl3_Main.c is foundation-locked.
