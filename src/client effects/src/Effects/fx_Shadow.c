...
static qboolean ShadowAddToView(client_entity_t* self, centity_t* owner)
{
	...
	//mxd. +CTF_CLIP_TO_BMODELS (fixes shadows disappearing when standing on brush entities (like func_wall or func_breakable)).
	//mxd. +CTF_LERP_BMODEL_ORIGIN (fixes shadows z-jittering when standing on a moving lift).
	fxi.Trace(start_pos, vec3_origin, vec3_origin, end_pos, CONTENTS_SOLID, CTF_CLIP_TO_WORLD | CTF_CLIP_TO_BMODELS | CTF_LERP_BMODEL_ORIGIN, &trace);
