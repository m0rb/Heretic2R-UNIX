#include "compat.h"
//
// vk_Model.c -- model loading and caching for the Vulkan renderer.
//
// Ported verbatim from gl3_Model.c (the validated H2 port of gl1_Model.c - semantics
// preserved 1:1): Mod_ForName dispatch (BSP brush models, H2 sprites, H2 book models,
// FlexModel .fm files), BSP lump loaders, inline models, PVS decompression,
// registration sequence handling. Hunk usage is identical to gl1 (shared Hunk.c).
// RI_GetReferencedID() moved here from gl1_Main.c (vk_Main.c is foundation-locked).
//
// Nothing here touches Vulkan directly - glpoly_t vertices use the vk_3D_vtx_t
// streaming layout (vk_Local.h; identical to gl3's gl3_3D_vtx_t), filled by the
// world module ports. Lightmap building (LM_*), warp surface subdivision
// (R_SubdivideSurface) and FlexModel block parsing (Mod_LoadFlexModel/
// Mod_RegisterFlexModel) are implemented by the world/flexmodel module ports -
// only the load dispatch lives here.
//
// Copyright 1998 Raven Software
//

#include "vk_Model_internal.h"
...
// gl1: defined in gl1_Main.c (see vk_Model_internal.h note). r_worldmodel itself
// is homed in vk_Main.c (foundation-locked) - only the view cluster trackers live here.
...
void RI_BeginRegistration(const char* map)
{
...
void RI_EndRegistration(void)
{
	model_t* mod = &mod_known[0];
	for (int i = 0; i < mod_numknown; i++, mod++)
		// logic inverted, freed already-empty slots (no-op) while stale named models leaked. --morb
...
// H2 //mxd. Named 'GetReferencedID' (in m_Reference.c) in original logic.
// gl1: lives in gl1_Main.c; moved here because vk_Main.c is foundation-locked.
int RI_GetReferencedID(const struct model_s* model)
{
