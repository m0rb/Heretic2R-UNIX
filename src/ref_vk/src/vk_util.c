#include "compat.h"
//
// vk_util.c -- Vulkan memory management: shared device memory blocks with
// suballocation, buffer/image resource creation and mapping helpers.
//
// Mechanical port of yquake2remaster vk_util.c (Copyright (C) 2018 Christoph
// Schied, (C) 2020 Denis Pauk, GPLv2) - yq2 refimport calls replaced with
// H2R's ri.* (CONTRACT.md).
//
				...
				VkMemoryPropertyFlags mem_skip)
{
	...
	// update preferred with required
	mem_pref |= mem_prop;
	...
	for (uint32_t i = 0; i < VK_MAX_MEMORY_TYPES; i++)
	{
		if (mem_req_type_bits & (1 << i))
		{
			...
			if ((propertyFlags & mem_prop) == mem_prop && !(propertyFlags & mem_skip))
			{
				int curr_cost;
				...
				// Remember memory type with upper cost as has more preferred bits.
...
memory_type_print(VkMemoryPropertyFlags mem_prop)
{
	if (!mem_prop)
	{
		ri.Con_Printf(PRINT_ALL, " VK_MEMORY_PROPERTY_NONE");
		return;
	}

#define MPSTR(r, prop) \
	if (((prop) & VK_MEMORY_PROPERTY_ ##r) != 0) \
		{ ri.Con_Printf(PRINT_ALL, " %s", "VK_MEMORY_PROPERTY_"#r); }
...
memory_block_empty(int* block_pos)
{
	...
	VK_CHECK_OOM(memory, "realloc() MemoryResource_t")
		...
		int* block_pos)
{
	...
	if (memory_block_empty(&pos) == VK_SUCCESS)
	{
		...
		if (suballocate)
		{
			...
			// allocate bigger memory for reuse
		...
		{
			ri.Con_Printf(PRINT_ALL, "%s:%d: VkResult verification: %s\n",
		...
		VkMemoryPropertyFlags* flags)
{
	...
	{
		ri.Con_Printf(PRINT_ALL, "Asked about memory properties with:\n");
	...
	{
		ri.Con_Printf(PRINT_ALL, "%s:%d: Have not found required memory type.\n",
			__func__, __LINE__);
		return VK_ERROR_OUT_OF_DEVICE_MEMORY;
	}
	else if (vk_validation->value > 0.0f)
	{
		ri.Con_Printf(PRINT_ALL, "%s:%d: Selected %d memory properties with:\n",
			__func__, __LINE__, memory_index);
		memory_type_print(vk_device.mem_properties.memoryTypes[memory_index].propertyFlags);
		ri.Con_Printf(PRINT_ALL, "\n");
	}
...
buffer_destroy(BufferResource_t* buf)
{
	...
	// buffer should be destroyed before bound memory
	...
	// buffer destroyed, we can free up memory
...
{
	// image should be destroyed before bound memory
	...
	// image destroyed, we can free up memory
