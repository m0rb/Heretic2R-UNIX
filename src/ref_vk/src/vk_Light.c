#include "compat.h"
//
// vk_Light.c -- dynamic lights, lightmap sampling and R_LightPoint().
//
// H2 semantics from gl1_Light.c (plus R_SetLightLevel() from gl1_Main.c) via
// the validated gl3_Light.c port: R_PushDlights() marks surfaces (dlightbits
// end up in the vk_3D_vtx_t lightFlags vertex attribute); R_BuildLightMap()
// copies the raw per-style H2 lightmap samples into the 4 style atlas buffers
// (lightstyle/gl_modulate scaling lives in the shader-side lmScales;
// gl_minlight remap kept CPU-side). gl_flashblend dlight coronas draw through
// vk_drawDLightPipeline (additive blend, no depth write - gl1
// glBlendFunc(GL_ONE, GL_ONE) + glDepthMask(GL_FALSE) parity).
//
// NOTE: the locked polygon_lmap shader set has no dynamic-light UBO (unlike
// gl3's uniLights block), so with gl_flashblend 0 the marked dlights are not
// yet applied to world surfaces - see the module report / vk_Surface.c
// R_BuildPolyVerts(): the lightFlags plumbing is in place for a future
// foundation shader extension.
//
...
extern qboolean r_minlight_set; // YQ2 (gl3: gl3state.r_minlight_set). Defined in vk_Misc.c, read by vk_FlexModel.c.
...
// Vertex layout of vk_drawDLightPipeline (RGB_RGB: pos + color, d_light.vert).
typedef struct
{
	vec3_t pos;
	float color[3];
...
// gl1: untextured additive triangle fan; vk: pos + per-vertex color fan
// through vk_drawDLightPipeline (pipeline/UBO/push constants bound by
// R_RenderDlights()).
static void R_RenderDlight(const dlight_t* light)
{
	vkdlightvtx_t verts[18];

	const float rad = light->intensity * 0.35f;

	vkdlightvtx_t* vtx = &verts[0];
	for (int i = 0; i < 3; i++)
	...
	QVk_DrawTriangleFan(verts, sizeof(vkdlightvtx_t), 18);
}
...
void R_RenderDlights(void)
{
	...
	// gl1: glDepthMask(GL_FALSE) + glBlendFunc(GL_ONE, GL_ONE) - baked into
	// vk_drawDLightPipeline. Coronas are drawn in world space: the d_light.vert
	// UBO carries the full mvp, which is just the view-projection here.
	QVk_BindPipeline(&vk_drawDLightPipeline);

	uint32_t ubo_offset;
	VkDescriptorSet ubo_set;
	uint8_t* ubo = QVk_GetUniformBuffer(sizeof(r_viewproj_matrix), &ubo_offset, &ubo_set);
	memcpy(ubo, r_viewproj_matrix, sizeof(r_viewproj_matrix));

	vkCmdBindDescriptorSets(vk_activeCmdbuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
		vk_drawDLightPipeline.layout, 0, 1, &ubo_set, 1, &ubo_offset);

	// basic_color_quad.frag grades in-shader - push the real grade (RP_WORLD
	// is graded per-fragment now, not in the postprocess blit).
	const float frag_push[3] = { vk_gradePush[0], vk_gradePush[1], vk_gradePush[2] };
	vkCmdPushConstants(vk_activeCmdbuffer, vk_drawDLightPipeline.layout, VK_SHADER_STAGE_FRAGMENT_BIT,
		PUSH_CONSTANT_VERTEX_SIZE * sizeof(float), sizeof(frag_push), frag_push);
...
// Q2 counterpart. gl3 also filled the uniLights UBO here; the locked vk
// polygon_lmap shader has no dynamic-light block (yet), so only the surface
// marking remains (dlightbits flow into the vk_3D_vtx_t lightFlags attribute
// for a future shader extension).
...
// gl3/vk (yq2): copies the raw per-style H2 lightmap samples into the 4 style atlas buffers.
// Unlike gl1, no lightstyle / gl_modulate scaling (shader-side lmScales), no dynamic lights
// and no channel-max rescale (raw bytes can't exceed 255) happen here;
