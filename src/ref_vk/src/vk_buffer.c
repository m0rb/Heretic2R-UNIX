#include "compat.h"
//
// vk_buffer.c -- Vulkan buffer creation: staging, uniform, vertex and index
// buffers (staged uploads through the transfer queue).
//
// Mechanical port of yquake2remaster vk_buffer.c (Copyright (C) 2018-2019
...
static VkResult
QVk_CreateBuffer(VkDeviceSize size, qvkbuffer_t* dstBuffer, const qvkbufferopts_t options)
{
	...
	return buffer_create(&dstBuffer->resource, bcInfo,
		options.reqMemFlags, options.prefMemFlags, /*skip memory*/ 0);
}

/* internal helper */
static void
createStagedBuffer(const void* data, VkDeviceSize size, qvkbuffer_t* dstBuffer, qvkbufferopts_t bufferOpts)
{
	qvkstagingbuffer_t* stgBuffer;

	stgBuffer = (qvkstagingbuffer_t*)malloc(sizeof(qvkstagingbuffer_t));
	VK_CHECK_OOM(stgBuffer, "malloc() qvkstagingbuffer_t")
...
QVk_CreateStagingBuffer(VkDeviceSize size, qvkstagingbuffer_t* dstBuffer,
		VkMemoryPropertyFlags reqMemFlags, VkMemoryPropertyFlags prefMemFlags)
{
	VkBufferCreateInfo bcInfo = {
		.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO,
		.pNext = NULL,
...
void
QVk_CreateVertexBuffer(const void* data, VkDeviceSize size, qvkbuffer_t* dstBuffer,
		VkMemoryPropertyFlags reqMemFlags, VkMemoryPropertyFlags prefMemFlags)
{
	qvkbufferopts_t dstOpts = {
...
void
QVk_CreateIndexBuffer(const void* data, VkDeviceSize size, qvkbuffer_t* dstBuffer,
		VkMemoryPropertyFlags reqMemFlags, VkMemoryPropertyFlags prefMemFlags)
{
	qvkbufferopts_t dstOpts = {
