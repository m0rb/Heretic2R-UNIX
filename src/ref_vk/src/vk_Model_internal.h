//
// vk_Model_internal.h -- model module interface (gl3_Model_internal.h counterpart for ref_vk).
//
// In-memory model representation shared with the world (vk_Surface.c / vk_Light.c /
// vk_Lightmap.c / vk_Warp.c), sky and flexmodel module ports. Structures are ported
// VERBATIM from gl1_Model.h / gl3_Model_internal.h, except glpoly_t whose vertices
// use the vk brush-surface streaming layout (vk_3D_vtx_t from vk_Local.h - identical
// to gl3's gl3_3D_vtx_t and to the MEM_VERTEX_T pipeline vertex input state).
//
...
// loader/renderer (vk_FlexModel.c) and by RI_GetReferencedID() (vk_Model.c).
//
...
// vk: vertices use the brush-surface streaming layout (vk_3D_vtx_t: pos, st,
// lightmap st, normal, lightFlags) instead of gl1's float[VERTEXSIZE] (xyz s1t1 s2t2) -
// gl3 parity (gl3_3D_vtx_t), matching the MEM_VERTEX_T vertex input state declared in
// vk_common.c CreatePipelines(). Filled by LM_BuildPolygonFromSurface() /
// R_SubdivideSurface() (world module ports), streamed through QVk_GetVertexBuffer().
typedef struct glpoly_s
{
	...
	int numverts;
	int flags;
	vk_3D_vtx_t vertices[4]; // Variable sized.
} glpoly_t;
...
#pragma region ========================== GLOBALS & PROTOTYPES (vk_Model.c) ==========================

extern int registration_sequence;

// gl1: defined in gl1_Main.c. r_worldmodel is homed in vk_Main.c (foundation:
// 'struct model_s* r_worldmodel;'); the view cluster trackers are homed in
// vk_Model.c (gl3 parity) - the world module (R_MarkLeaves / R_SetupFrame ports)
// externs them from here.
...
// in vk_Local.h (module function set).

#pragma endregion

#pragma region ========================== CROSS-MODULE PROTOTYPES (implemented elsewhere) ==========================

// --- vk_Lightmap.c (world module port; signatures mirror gl1_Lightmap.h) ---
...
// --- vk_Warp.c (world module port; signature mirrors gl1_Warp.h) ---
extern void R_SubdivideSurface(const model_t* mdl, msurface_t* fa); //mxd. Added 'mdl' arg.

// --- vk_FlexModel.c (flexmodel module port; signatures mirror gl1_FlexModel.h) ---
