#include "compat.h"
//
// vk_validation.c -- Vulkan validation layer callbacks (VK_EXT_debug_utils /
// VK_EXT_debug_report), enabled with the vk_validation cvar.
//
// Mechanical port of yquake2remaster vk_validation.c (Copyright (C) 2018-2019
...
{
	const int g = (type & VK_DEBUG_UTILS_MESSAGE_TYPE_GENERAL_BIT_EXT) != 0 ? 1 : 0;
	const int p = (type & VK_DEBUG_UTILS_MESSAGE_TYPE_PERFORMANCE_BIT_EXT) != 0 ? 1 : 0;
	const int v = (type & VK_DEBUG_UTILS_MESSAGE_TYPE_VALIDATION_BIT_EXT) != 0 ? 1 : 0;
