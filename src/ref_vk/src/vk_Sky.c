#include "compat.h"
//
// vk_Sky.c -- skybox clipping and rendering.
//
// H2 semantics from gl1_Sky.c via the validated gl3_Sky.c port (r_farclipdist
// / fog-dependent skybox scale, pics/skies/*.m8 sky faces, skyrotate-dependent
// seam clamp); draw technique from yq2 vk_warp.c R_DrawSkyBox() (MakeSkyVec
// into vk_3D_vtx_t quads, model matrix rotate + translate in the per-draw
// UBO, vk_drawSkyboxPipeline).
//
...
static void R_MakeSkyVec(float s, float t, const int axis, vk_3D_vtx_t* vert) // YQ2: fills a vertex instead of glTexCoord2f() + glVertex3fv().
{
	// 1 = s, 2 = t, 3 = 2048
	...
	if ((int)r_fog->value) //mxd. Removed gl_fog_broken cvar check.
		...
		clipdist = r_farclipdist->value * 0.5773503f; //TODO: what's with the scaler? (1/sqrt(3): box corners end up at farclip distance --mxd)
	...
	vert->texCoord[0] = s;
	vert->texCoord[1] = 1.0f - t;
	vert->lmTexCoord[0] = 0.0f;
	vert->lmTexCoord[1] = 0.0f;
	VectorSet(vert->normal, 0.0f, 0.0f, 0.0f);
	vert->lightFlags = 0;
}
...
void R_DrawSkyBox(void)
{
	...
	// gl1: glPushMatrix() + glTranslatef(r_origin) + glRotatef(time * skyrotate, skyaxis). // YQ2
	// (Row-vector composition: rotate in sky space first, then move to the view origin;
	// world sky is always drawn in world space, so r_local_model_matrix is not involved.)
	float model[16];
	Mat4_Identity(model);

	if (skyrotate != 0.0f)
		Mat4_Rotate(model, r_newrefdef.time * skyrotate, skyaxis[0], skyaxis[1], skyaxis[2]);

	Mat4_Translate(model, r_origin[0], r_origin[1], r_origin[2]);

	// skybox.vert per-draw UBO (set 1, binding 0): just the model matrix.
	uint32_t ubo_offset;
	VkDescriptorSet ubo_set;
	uint8_t* ubo = QVk_GetUniformBuffer(sizeof(model), &ubo_offset, &ubo_set);
	memcpy(ubo, model, sizeof(model));

	QVk_BindPipeline(&vk_drawSkyboxPipeline);
	QVk_PushMatrix(&vk_drawSkyboxPipeline, r_viewproj_matrix);
	QVk_PushWorldFragmentConstants(&vk_drawSkyboxPipeline, -1.0f); // Real grade, no alpha test.

	vk_3D_vtx_t sky_vertices[4];
	...
	for (int i = 0; i < 6; i++)
	{
		...
		{
			const image_t* image = sky_images[skytexorder[i]];
			if (!R_ImageUsable(image)) //mxd. Defensive check (missing sky face pics / vk_Image.c module-port ordering).
				continue;

			const VkDescriptorSet desc_sets[] = { image->vk_texture.descriptorSet, ubo_set };
			vkCmdBindDescriptorSets(vk_activeCmdbuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
				vk_drawSkyboxPipeline.layout, 0, 2, desc_sets, 1, &ubo_offset);

			R_MakeSkyVec(skymins[0][i], skymins[1][i], i, &sky_vertices[0]);
			R_MakeSkyVec(skymins[0][i], skymaxs[1][i], i, &sky_vertices[1]);
			R_MakeSkyVec(skymaxs[0][i], skymaxs[1][i], i, &sky_vertices[2]);
			R_MakeSkyVec(skymaxs[0][i], skymins[1][i], i, &sky_vertices[3]);

			QVk_DrawTriangleFan(sky_vertices, sizeof(vk_3D_vtx_t), 4); // gl1: GL_QUADS.
		}
	}
}
